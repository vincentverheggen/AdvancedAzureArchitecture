...
namespace GameAPI.Actors
{
    public class SessionActor : Actor, ISessionActor
    {
        ...
        public SessionActor(ActorHost host, [FromServices] IEmailService emailService, [FromServices] ILeaderboardService leaderboardService, IConfiguration configuration) : base(host)
        {
        ...
        async Task EndGame(Session session)
        {
            Stats.Timestamp = DateTime.UtcNow;
            await _leaderboardService.SendSessionResult(Stats);
